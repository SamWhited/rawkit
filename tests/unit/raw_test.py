...
def test_get_col_4_raw(raw, mock_ctypes):
    raw.data.contents.sizes.pixel_aspect = 1
    raw.data.contents.sizes.flip = 0

    result = raw.get_4_col_raw()

    assert result is not None


def test_get_col_3_raw(raw, mock_ctypes):
    raw.data.contents.sizes.pixel_aspect = 1
    raw.data.contents.sizes.flip = 0

    result = raw.get_3_col_raw()

    assert result is not None


def test_get_col_4_bad_aspect(raw, mock_ctypes, mock_warning):
...
def test_get_col_3_bad_aspect(raw, mock_ctypes, mock_warning):
    raw.data.contents.sizes.pixel_aspect = 2
    raw.data.contents.sizes.flip = 0
